...
- IP 查询 route 仅作 adapter；统一 implementation 位于 `src/modules/query`，旧输出 projection 位于 `legacy.ts`。字段变化需覆盖主页面、兼容 GET/POST 与 `/api/ip/:ip`，见 [架构说明](docs/architecture.md)。
