...
provider JP 上原有的 `18080` 服务是本仓库定制版 NextTrace 的 deploy REST API，不是独立部署的 GlobalTrace 服务。公开 base URL 为：
